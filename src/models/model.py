...
from src.models.functions import gen_ztta
...
class Model:
    def __init__(self,load_pre_trained=True):
        ...
        self.skeleton_mocap = Skeleton(offsets=self.test_configrations['data']['offsets'], parents=self.test_configrations['data']['parents'])
    ...
    def load_components(self):
        ...
        lstm = LSTM(in_dim=self.test_configrations['model']['lstm_dim'], hidden_dim=self.test_configrations['model']['lstm_dim'] * 2)
        self.lstm = lstm.cuda()
        decoder = Decoder(in_dim=self.test_configrations['model']['lstm_dim'] * 2, out_dim=self.test_configrations['model']['state_input_dim'])
    ...
    def load_pre_trained(self):
        self.state_encoder.load_state_dict(torch.load(os.path.join(self.test_configrations['test']['model_dir'], 'state_encoder.pkl')))
        self.offset_encoder.load_state_dict(torch.load(os.path.join(self.test_configrations['test']['model_dir'], 'offset_encoder.pkl')))
        self.target_encoder.load_state_dict(torch.load(os.path.join(self.test_configrations['test']['model_dir'], 'target_encoder.pkl')))
        self.decoder.load_state_dict(torch.load(os.path.join(self.test_configrations['test']['model_dir'], 'decoder.pkl')))
    ...
    '''
    returns two tuples one for lists and the other one for losses
    '''
    def predict(self, batch_dict, seq_length=50):
        ...
        for t in range(seq_length):
            ...
            lambda_target=self._get_labmda(t)
        ...
        return (pred_list,contact_list) , (loss_pos,loss_quat,loss_contact,loss_root)

    def _get_labmda(self,t):
