...
from src.features.skeleton import Skeleton
        ...
        for i_batch, sampled_batch in enumerate(lafan_loader_test):
            ...
            with torch.no_grad():
                ...
                for t in range(opt['model']['seq_length'] - 1):
                    ...
                    r="/content/drive/MyDrive/Pytorch-Robust-Motion-In-betweening/results"
                    if opt['test']['save_img']:
                        if os.path. exists(r+version+'/pred') == False :
                          os.mkdir(r+version+'/pred')
                        if os.path. exists(r+version+'/gt') == False :
                          os.mkdir(r+version+'/gt')
                        ...
                        plot_pose(np.concatenate([X[bs,0].view(22, 3).detach().cpu().numpy(),\
                                                ...
                                                X[bs,-1].view(22, 3).detach().cpu().numpy()], 0),\
                                                t, r+version+'/gt')
                        pred_img = Image.open(r+version+'/pred_'+str(t)+'.png', 'r')
                        gt_img = Image.open(r+version+'/gt_'+str(t)+'.png', 'r')
                ...
                if opt['test']['save_bvh']:
                    ...
                    # assert 0
                    write_to_bvhfile(bvh_data, ('/content/drive/MyDrive/Pytorch-Robust-Motion-In-betweening/bvh_seq/test_%03d.bvh' % i_batch), opt['data']['joints_to_remove'])
                    # assert 0
                    ...
                    glb = remove_fs(('/content/drive/MyDrive/Pytorch-Robust-Motion-In-betweening/bvh_seq/test_%03d.bvh' % i_batch), \
                              ...
                              fid_l=(3, 4), \
                              fid_r=(7, 8),\
                              output_path=("/content/drive/MyDrive/Pytorch-Robust-Motion-In-betweening/bvh_seq_after"+version+"/test_%03d.bvh" % i_batch))
                    # fix_img_list = []
