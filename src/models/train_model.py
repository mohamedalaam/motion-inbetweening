            ...
            if True:
                ...
                for t in range(lafan_data_train.cur_seq_length - 1):
                    ...
                    loss_pos += torch.mean(torch.abs(
                        pos_pred[0] - pos_next) / x_std) / lafan_data_train.cur_seq_length  # opt['model']['seq_length']
                    loss_quat += torch.mean(torch.abs(
                        local_q_pred[0] - local_q_next)) / lafan_data_train.cur_seq_length  # opt['model']['seq_length']
                    loss_root += torch.mean(torch.abs(root_pred[0] - root_p_next) / x_std[:, :,
                                                                                    0]) / lafan_data_train.cur_seq_length  # opt['model']['seq_length']
                    loss_contact += torch.mean(torch.abs(
                        contact_pred[0] - contact_next)) / lafan_data_train.cur_seq_length  # opt['model']['seq_length']
                ...
                if opt['train']['use_adv']:
                    ...
                    real_input = torch.cat(
                        [X[:, i].view(X.size(0), -1).unsqueeze(-1) for i in range(lafan_data_train.cur_seq_length)], -1)
